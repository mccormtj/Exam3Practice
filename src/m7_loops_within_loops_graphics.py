...
def hourglass(window, n, point, radius, color):
    ...
    for k in range(n+1):
        for j in range(k):
            ...
            line = rg.Line(rg.Point(circle.center.x - radius, circle.center.y), rg.Point(circle.center.x + radius,
                                                                                         circle.center.y))
...
def many_hourglasses(window, square, m, colors):
    ...
    for k in range(m):
        ...
        if n == len(colors):
            n = 0
        hourglass(window, k + 1, center, radius, colors[n])
        x1 = x2
        x2 = x2 + side_length*(k + 2)
